...
use crate::models::{
  Pool, PreSignRequest, PreSignResponse, PublicKeyResponse, SignRequest, SignResponse, Signature,
  SlimUser, User,
};
use rand::thread_rng;
...
pub fn pre_request_sign(
  ...
  match id.identity().as_ref() {
    Some(identity) => {
      ...
      let x = PreSignResponse { x: user.x };
    ...
    Some(identity) => {
      ...
      match BigUint::from_str(&user.x) {
        Ok(x) => {
          ...
          Either::A(web::block(move || query_sign(user, pool)).then(
            move |res: Result<SlimUser, BlockingError<ServiceError>>| match res {
              Ok(_) => Ok(HttpResponse::Ok().body(serde_json::to_string(&signature).unwrap())),
              Err(err) => match err {
        ...
        Err(_) => Either::B(err(ServiceError::NoPresign)),
      }
    }
...
fn query_sign(auth_data: SlimUser, pool: web::Data<Pool>) -> Result<SlimUser, ServiceError> {
  use crate::schema::users::dsl::{has_voted, username, users};
